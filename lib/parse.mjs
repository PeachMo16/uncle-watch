...
export function parseFiling(xml, meta = {}) {
  ...
  const transactions = [...xml.matchAll(/<nonDerivativeTransaction>[\s\S]*?<\/nonDerivativeTransaction>/g)].map((m) => m[0]).map((t) => {
    ...
    return {
      ...
      directOrIndirect: val(t, 'directOrIndirectOwnership'),
...
export function parseFiles(files) {
  return files.map((f) => parseFiling(readFileSync(f.out ?? f, 'utf8'), {
    ...
    acceptanceDateTime: f.acceptanceDateTime, url: f.url,
  }));
}

// An amended ownership form refiles (and may correct) its original form type.
// Form 3/A and 5/A are kept separate from Form 4 even for the same owner/period.
...
export function dedupeAmendments(filings) {
  ...
  for (const fl of filings) {
    const key = `${(fl.form ?? '4').replace(/\/A$/, '')}|${fl.issuerCik}|${fl.owners.map((o) => o.cik).sort().join('+')}|${fl.periodOfReport}`;
    (groups[key] ??= []).push(fl);
  }
  return Object.values(groups).flatMap((g) => {
    const amends = g.filter((f) => /\/A$/.test(f.form ?? ''));
    ...
    const kept = g.filter((f) => !/\/A$/.test(f.form ?? '') && !byOrigDate[f.filingDate]);
...
export function aggregateInsiders(filings) {
  ...
  for (const fl of filings) {
    for (const o of fl.owners) {
      const key = o.cik || `name:${o.name}`;
      insiders[key] ??= { name: o.name, cik: o.cik, aliases: new Set(), roles: new Set(), events: [] };
      insiders[key].aliases.add(o.name);
