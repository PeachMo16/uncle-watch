...
<text x="${x}" y="${y + 15}" text-anchor="${anchor}" fill="${col}" font-size="13" font-weight="bold">${d.score}</text>`;
  ...
  // counter-signals (e.g. open-market buying) are opposite-direction evidence:
  // one green line each, never an axis — a radar of same-direction risk axes stays honest
  const counters = (rate.counterSignals ?? []).map((c, i) =>
    `<text x="${W / 2}" y="${H - 16 - i * 18}" text-anchor="middle" fill="#3fb950" font-size="12">${c.observed ? '↑' : '·'} ${c.label}: ${c.evidence}</text>`
  ).join('\n');
