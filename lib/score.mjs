...
export function uncleRate(report, sub) {
  ...
  const nonPlan = sells.filter((s) => s.planStatus !== '10b5-1 indicated');
  ...
  // than the last, whole run inside 10 calendar days
  let ladder = null;
  const byInsider = {};
  for (const s of nonPlan) (byInsider[s.insider] ??= []).push(s);
  for (const [name, ss] of Object.entries(byInsider)) {
    ...
    for (let i = 0; i < knownPct.length; i++) {
      ...
      if (!ladder || peak > ladder.peak) ladder = { name, peak, steps: run.map((s) => s.pctOfStake), from: run[0].date, to: run[run.length - 1].date };
    }
  }
  dims.push({
    ...
    evidence: ladder
      ? `${ladder.name} escalated ${ladder.steps.join('% → ')}% of stake, ${ladder.from} → ${ladder.to}`
      : 'no escalating multi-day sell ladders',
  });

  // 4 · exit discipline — tightest personal price band across ≥3 non-plan sells
  let disc = null;
  for (const [name, ss] of Object.entries(byInsider)) {
  ...
  dims.push({
    ...
    evidence: disc
      ...
      : 'no insider with ≥3 sells lacking a 10b5-1 indication',
  });
  ...
  return { scoringVersion: 2, composite, dims, counterSignals };
}
