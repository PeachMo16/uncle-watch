...
export function uncleRate(report, sub) {
  ...
  const maxIns = Math.max(0, ...report.clusters.map((c) => new Set(c.map((x) => x.insiderCik || x.insider)).size));
  ...
  let belowFloor = 0;
  const byInsider = {};
  for (const s of noInd) (byInsider[s.insiderCik || s.insider] ??= []).push(s);
  for (const ss of Object.values(byInsider)) {
    const name = ss[0].insider;
  ...
  const buyers = new Set(buys.map((b) => b.insiderCik || b.insider)).size;
